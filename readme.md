...
## Rules
This is a WIP list of rules. Each day uses some set of them. If a rule is not in effect on a given day, mail matching that rule in the player queue will be handled automatically.

Mail must be sent to recipient: Valid mail addressed to the player's town must be sent to the recipient. (This rule can not be off)
Mail must be routed appropriately: Valid mail must be routed to the appropriate neighboring post office.
Mail must be stamped: Mail without a postal stamp must be RTS'd as "Postage Due"
Stamps must be sufficient: Mail with a stamp not appropriate for routing to another town must be RTS'd as "Postage shortpaid"
...
- A sub-folder called "story" which contains the specification for the different rulesets and on which days they apply
